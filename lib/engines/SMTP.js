...
exports.send = function(emailMessage, config, callback) {
    ...
    var client = new SMTPClient(emailMessage.SERVER.host, emailMessage.SERVER.port, {
            ...
            xoauth: emailMessage.SERVER.xoauth,
...
function SMTPClient(host, port, options){
    ...
    this.remote_auth_xoauth = false;    // Does the server support CRAM-SHA1
...
SMTPClient.prototype._flushMessages = function() {
    ...
    if(!mail.fromAddress || !mail.fromAddress.length){
        this.emit("error",new Error("Sender address not defined"));
        return;
    }
    
    if(!mail.toAddress || !mail.toAddress.length){
        this.emit("error",new Error("No recipients defined (at least one address required for To, Cc or Bcc field"));
        return;
    }
    
    if(this.debug && !mail.subject){
        console.log("▒▒▒ WARNING: Empty subject line ▒▒▒");
    }
...
SMTPClient.prototype._loginHandler = function(callback){
    if(!this.options.use_authentication){
        ...
        if(this.remote_auth_xoauth){
            // use AUTH XOAUTH
            this._xauthLogin(callback);
        }else if(this.remote_auth_plain){
            // use AUTH PLAIN
 ...
 * - callback (Function): function to be run after successful login
 *
 * Performs an AUTH XOAUTH login
 * 
 * C: AUTH XOAUTH <SASL initial client request>
 **/
SMTPClient.prototype._xauthLogin = function(callback){
    this.send("AUTH XOAUTH "+this.options.xoauth, (function(error, message){
        if(error){
